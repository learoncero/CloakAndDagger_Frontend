...
// Sprite Components
const PlayerSprites: React.FC<PlayerSpritesProps> = ({
  player,
  currentPlayerRole,
}: PlayerSpritesProps) => {
  const [isGhost, setIsGhost] = useState(
    player.role === Role.IMPOSTOR_GHOST || player.role === Role.CREWMATE_GHOST
  );
  ...
  const isOtherPlayerImpostor =
    player.role === Role.IMPOSTOR_GHOST || player.role === Role.IMPOSTOR;
