...
export default function Voting ({currentPlayer, activePlayers, handleVotes}: VotingProps) {
                        ...
                        {activePlayers.map((player) =>
                            <div key={player.id}>
                                <input
                                    ...
                                    className={"scale-110 transform hover:scale-125 disabled:text-gray-400"}
                                />
                                <label htmlFor={`${player.id}`}
                                       className={`ml-2 checked:text-cyan-500 ${selectedPlayerId === player.id ? 'text-cyan-500' : 'text-white'}`}>
                                    {player.username}
                                </label>
                            </div>)}
